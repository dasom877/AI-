...
const PREV_DEFAULT_WEBHOOK = "https://wh.jandi.com/connect-api/webhook/27388464/6e67c46677fd21750badc363ae2393d9";
const DEFAULT_WEBHOOK = "https://wh.jandi.com/connect-api/webhook/27388464/e1452e890e3d5a5ad99b5429cfde65cd";

const DEFAULT_WEBHOOKS: JandiWebhookChannel[] = [
  { id: "channel-1", name: "📢 [인사팀] 온보딩 총괄 알림방", url: "https://wh.jandi.com/connect-api/webhook/27388464/e1452e890e3d5a5ad99b5429cfde65cd" },
  { id: "channel-2", name: "💛 [버디/루키] 13기 소통 채널", url: "https://wh.jandi.com/connect-api/webhook/27388464/7d45146733fa60e8ed6d0f3c188b0a3a" },
  { id: "channel-3", name: "🧪 [테스트] 개인 수신/디버깅 채널", url: "https://wh.jandi.com/connect-api/webhook/27388464/e1452e890e3d5a5ad99b5429cfde65cd" }
];
...
export function getJandiWebhooks(): JandiWebhookChannel[] {
  ...
  try {
    ...
    let migrated = false;
    const updated = parsed.map((c: JandiWebhookChannel) => {
      // Migrate old default URLs to the new requested URL
      if (c.url === PREV_DEFAULT_WEBHOOK) {
        migrated = true;
        return { ...c, url: DEFAULT_WEBHOOK };
      }
      return c;
    });
    if (migrated) {
      localStorage.setItem("jandi_webhooks", JSON.stringify(updated));
    }
    return updated;
  } catch {
    return DEFAULT_WEBHOOKS;
