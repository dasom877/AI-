...
export default function GoogleSheetsSync({
  ...
  const fetchSpreadsheetData = async (targetId: string) => {
    ...
    try {
      ...
      const mappedSurvey: SatisfactionSurvey[] = surveySheet.flatMap((row) => {
        const keys = Object.keys(row);
        const getVal = (possibleKeys: string[]) => {
          const foundKey = keys.find((k) =>
            possibleKeys.some((pk) => k.toLowerCase().replace(/\s+/g, "").includes(pk))
        ...
        const respondent = String(getVal(["응답자", "respondent", "이름"]) || "익명");
        const department = String(getVal(["부서", "dept", "소속", "team"]) || "기타");

        const surveyEntries: SatisfactionSurvey[] = [];
        keys.forEach((key) => {
        ...
        if (surveyEntries.length === 0) {
          const qNo = String(getVal(["번호", "문항번호", "qno"]) || "Q1");
          const question = String(getVal(["문항", "내용", "문항내용", "question"]) || "주제");
          const response = String(getVal(["응답", "점수", "답변", "response"]) || "");
          if (response) {
            surveyEntries.push({ cohort, respondent, department, qNo, question, response });
          }
        }
